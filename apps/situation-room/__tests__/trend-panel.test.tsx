...
describe('TrendPanel', () => {
  ...
  it('renders the trend subsection with semantic x-axis context', async () => {
    ...
    expect(container.firstElementChild?.getAttribute('class')).toContain('w-full');
    expect(
      container.querySelector('[data-testid="trend-chart"]')?.parentElement?.getAttribute('class'),
    ).toContain('flex-1');
    expect(
      container.querySelector('[data-testid="trend-chart"]')?.parentElement?.getAttribute('class'),
    ).toContain('min-h-0');
