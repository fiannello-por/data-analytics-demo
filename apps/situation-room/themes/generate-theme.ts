...
export function generateCssFromTheme(theme: Theme): string {
  ...
  // 7. Component-resolved geometry (NEW — extends current CSS)
  if (theme.components) {
    const { radiusBase, radiusScale, shadow } = geometry;
    for (const [comp, config] of Object.entries(theme.components)) {
      for (const [key, value] of Object.entries(config)) {
        ...
        if (key === 'radius') {
          ...
          // railRadius → rail-radius
          const stem = kebabCase(key);
          const scale = radiusScale[strValue];
          if (scale !== undefined) {
            const resolved =
              scale === 1 ? radiusBase : `calc(${radiusBase} * ${scale})`;
          ...
          const resolved = shadow[strValue];
          if (resolved)
            lines.push(`  --${kebabCase(comp)}-shadow: ${resolved};`);
        } else if (key === 'height') {
          lines.push(`  --${kebabCase(comp)}-height: ${strValue};`);
        }
      }
    }
  }
...
export function generateThemeInlineBlock(
  ...
  // 5. Component-resolved geometry → Tailwind utilities
  if (componentVarNames.length > 0) {
    lines.push('');
    lines.push('  /* component geometry \u2192 Tailwind mappings */');
    for (const varName of componentVarNames) {
      if (varName.match(/^--[\w-]+-radius$/)) {
        const name = varName.replace(/^--/, '').replace(/-radius$/, '');
        lines.push(`  --radius-${name}: var(${varName});`);
      } else if (varName.match(/^--[\w-]+-[\w-]+-radius$/)) {
        // e.g. --tab-rail-radius → --radius-tab-rail
        const name = varName.replace(/^--/, '').replace(/-radius$/, '');
        lines.push(`  --radius-${name}: var(${varName});`);
      } else if (varName.match(/^--[\w-]+-shadow$/)) {
        const name = varName.replace(/^--/, '').replace(/-shadow$/, '');
        lines.push(`  --shadow-${name}: var(${varName});`);
      }
      // --*-height vars are available as plain CSS vars, no Tailwind mapping needed
    }
  }

  // 6. Font mono (last, matching global.css order)
...
function generate(themesDir: string, outputPath: string): void {
  ...
  // Collect component geometry var names
  const componentVarNames: string[] = [];
  if (lightTheme.components) {
    for (const [comp, config] of Object.entries(lightTheme.components)) {
      for (const [key, value] of Object.entries(config)) {
        if (key === 'radius') {
          componentVarNames.push(`--${kebabCase(comp)}-radius`);
        } else if (key.endsWith('Radius')) {
          componentVarNames.push(`--${kebabCase(comp)}-${kebabCase(key)}`);
        } else if (key === 'shadow') {
          componentVarNames.push(`--${kebabCase(comp)}-shadow`);
        } else if (key === 'height') {
          componentVarNames.push(`--${kebabCase(comp)}-height`);
        }
      }
    }
  }
